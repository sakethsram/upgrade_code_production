...
def execute_show_commands(device_key, vendor, model, conn, check_type, logger):
    ...
    if not commands:
        logger.warning(f"[{device_key}] execute_show_commands — no commands loaded, aborting")
        return False
...
def validate_device_dict(data: dict, device_key, show_cmds,  logger):
    """
    ...
    for key, expected_type in required_keys.items():
        if key not in data:
            ...
            if key == "host": 
                logger.warning(f"[{device_key}] : [DEVICE_NOT_FOUND ] Specified device could not be located")
                
            logger.warning(f"[{device_key}] : Missing required key: {key}")
        else:
            if not isinstance(data[key], expected_type):
                errors.append(
                    f"Invalid type for {key}: expected {expected_type.__name__}, got {type(data[key]).__name__}"
                )
                logger.warning(f"[{device_key}] : Invalid type for {key}: expected {expected_type.__name__}, got {type(data[key]).__name__}")
    ...
    if "imageDetails" in data and isinstance(data["imageDetails"], list):
        for idx, entry in enumerate(data["imageDetails"], start=1):
            if not isinstance(entry, dict):
                ...
                errors.append(msg)
                logger.warning(f"[{device_key}] : {msg}")
                continue
            ...
            if data.get("intermediate_release", False):
                if "intermediate_image" not in entry and idx == 1:
                    ...
                    if not os.path.isdir(data["image_path"]): 
                        msg = f"image_path directory not found: {data['image_path']}"
                        errors.append(msg)
                        logger.warning(f"[{device_key}] : {msg}")
                    else: 
                        if "intermediate_image" in entry:
                          image_file = os.path.join(data["image_path"], entry["intermediate_image"])
                          if not os.path.isfile(image_file):
                              msg = f"imageDetails[{idx}] [FILE_NOT_FOUND] : The specified file could not be found: {image_file}"
                              errors.append(msg)
                              logger.warning(f"[{device_key}] : {msg}")
                
                if "image" not in entry and idx == 2: 
                    msg = f"image[{idx}] missing 'image' for target image"
                    errors.append(msg) 
                    logger.warning(f"[{device_key}] : {msg}")
                else: 
                    if "image" in entry:
                        if not os.path.isdir(data["image_path"]): 
                            ...
                            image_file = os.path.join(data["image_path"], entry["image"])
                            if not os.path.isfile(image_file):
                                msg = f"imageDetails[{idx}] [FILE_NOT_FOUND] : The specified file could not be found: {image_file}"
                                errors.append(msg)
                                logger.warning(f"[{device_key}] : {msg}")
            else: 
                if "image" not in entry and idx == 1: 
                    msg = f"image[{idx}] missing 'image' for target image"
                    errors.append(msg) 
                    logger.warning(f"[{device_key}] : {msg}")
                else: 
                    ...
                    if not os.path.isdir(data["image_path"]): 
                        ...
                        if "smu_image_path" in data and data["smu_image_path"] is not None:
                            if not isinstance(data["smu_image_path"], str):
                                ...
                                errors.append(msg)
                                logger.warning(f"[{device_key}] : {msg}")
                            elif not os.path.isdir(data['smu_image_path']): 
                                msg = f"smu_image_path directory not found: {data['smu_image_path']}"
                                errors.append(msg)
                                logger.warning(f"[{device_key}] : {msg}")
                            else: 
                                for smu_idx, smu_entry in enumerate(entry["smu_images"], start=1):
                                    smu_file = os.path.join(data["smu_image_path"], smu_entry["image"])
                                    if not os.path.isfile(smu_file):
                                        msg = f"imageDetails[{idx}].smu_images[{smu_idx}] [FILE_NOT_FOUND] : The specified file could not be found: {smu_file}"
                                        errors.append(msg)
                                        logger.warning(f"[{device_key}] : {msg}")
    ...
    # Validate the show cmd list yaml file 
    vendor_model = f"{data.get('vendor', '')}_{data.get('model', '')}"
    if vendor_model in show_cmds: 
        commands = show_cmds[vendor_model]
        if not commands or not isinstance(commands, list): 
            msg = f"No show commands found for {vendor_model}"
            errors.append(msg) 
            logger.warning(f"[{device_key}] : {msg}")
    else: 
        msg = f"Vendor / model {vendor_model} not found in command mapping"
        errors.append(msg) 
        logger.warning(f"[{device_key}] : {msg}")
...
# Phase 1
# ─────────────────────────────────────────────────────────────────────────────
def run_prechecks(conn, dev: dict, device_key: str, models: list, show_cmds: list, logger):
    ...
    try:
        ...
        validate_input = validate_device_dict(dev, device_key, show_cmds, logger)
        ...
        if not exec_ok:
            msg = f"[{device_key}]: execute_show_commands() failed (collections/parsing)"
        ...
        for cmd in commands: 
            if "version" in cmd.get("cmd") :
                version_json = cmd.get("json")
                break
                
         
        if not version_json:
          logger.warning(f"[{device_key}] : show version output not found in device_results")
          return False
        ...
        if vendor_lc == "cisco":
          validate_FPD  = precheck.validateFPDs(conn, logger, device_key, "pre")
          if not validate_FPD:
            msg = f"[{device_key}]: validateFPDs() failed"
          ...
          if not check_auto_fpd:
            msg = f"[{device_key}]: check_auto_fpd() failed"
    ...
    try:
        ...
        for i, details in enumerate(image_details):
            ...
            if not image or not expected_os or not checksum:
                msg = (
                    f"[{device_key}]: imageDetails[{i}] missing one of: "
            ...
            if not storage:
                msg = f"[{device_key}]: checkStorage() failed for image transfer. Please clean up the device to transfer image into device for upgrade."
            ...
            if not storage:
                msg = f"[{device_key}]: checkStorage() failed for Upgrade"
            ...
            if vendor =="cisco" :
              ...
              if model in asr_models:
                ...
                logger.info(f"[{device_key}] : Hostname->{hostname}")
                if "OSA" in hostname or "TYO" in hostname:
                  apply_deny_any=upgrade.apply_deny_policy(conn,logger)
                  if not apply_deny_any:
                    logger.info(f"[{device_key}] : Failed to apply_deny_any policy to neighbors")
                    return False
     
                logger.info(f"[{device_key}] : Hostname does not contain OSA or TYO")
                conn.clear_buffer()
              ...
              logger.info(f"[{device_key}]  Overload bit Set {set_overload_bit.get('status')}")
    ...
    try: 
        ...
        for j,smu in enumerate(smu_list): 
            ...
            if not image or not checksum:
                msg = (
                    f"[{device_key}]: imageDetails[{j+1}] missing one of: "
                    f"image, checksum"
                )
                logger.error(f"[{device_key}] {msg}")
                device_results[device_key]["upgrade"]["status"]    = "failed"
...
# run_rollback
# ─────────────────────────────────────────────────────────────────────────────
def run_rollback(conn, device: dict, device_key: str, accepted_vendors: list, logger, models):
    host        = device.get("host")
    ...
    logger.info(
        ...
        f"curr_os={curr_os}")
    logger.info(f"[{device_key}] Running Rollback...")
...
def run_postchecks(conn, dev: dict, device_key: str, logger, models):
    ...
    try:
        ...
        logger.info(f"[{device_key}] POST : show version")
        ...
        logger.info(
            f"[{device_key}] [post] show_version parsed — "
            f'hostname={version_json.get("hostname")}  model={version_json.get("model")}  version={version_json.get("version")}'
        )
        ...
        if vendor_lc == "cisco": 
            ...
            validate_FPD  = precheck.validateFPDs(conn, logger, device_key, "post")
            if not validate_FPD:
              msg = f"[{device_key}]: validateFPDs() failed"
              logger.error(f"[{device_key}] validateFPDs failed — {msg}")
            ...
            if not remove_overload_bit:
              ...
              device_results[device_key]["post"]["remove_overload_bit"]["exception"] = msg
              return False
            device_results[device_key]["post"]["remove_overload_bit"] = remove_overload_bit
...
# ─────────────────────────────────────────────────────────────────────────────
def run_device_pipeline(dev: dict, accepted_vendors: list, models: list, show_cmds: list):
    ...
    try:
        ...
        if vendor not in accepted_vendors:
            msg = (
                f"[{device_key}] [VENDOR_MODEL_NOT_FOUND] Specified vendor or model not found in supported list — {accepted_vendors}"
            )
            logger.warning(msg)
        ...
        precheck_ok = run_prechecks(conn, dev, device_key, models, show_cmds, logger)
        if not precheck_ok:
        ...
        if vendor == "cisco" and smu_upgrade: 
            conn, is_smu_upgrade = run_smu_upgrade(conn, dev, device_key, accepted_vendors, logger, models)
            device_results[device_key]["conn"] = conn
...
#            )
        ...
        logger.error(f"[{device_key}] [WORKFLOW_ABORTED] : The workflow was aborted before completion")
        raise
...
def main():
    ...
    show_cmds = load_yaml("show_cmd_list.yaml") 
    if not show_cmds: 
        logger.warning("[EMPTY_FILE] : The file is empty and cannot be processed")
        sys.exit(0)
    ...
    try:
        with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
            futures = {
                executor.submit(run_device_pipeline, dev, accepted_vendors, models, show_cmds): dev
                for dev in all_devs
            }
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"[THREAD_EXECUTION_ERROR] Thread execution error {dev.get('host')}: {e}")
    except KeyboardInterrupt:
        print("[WORKFLOW_ABORTED] : The workflow was aborted before completion")
